<?php
    ...
    //date diff
    $factura = date_create("2024-10-10");
    echo "<br>fecha de la factura es: " . date_format($factura, "Y-m-d");
    $hoy = date_create("now");
    echo "<br>La fecha de hoy es: " . date_format($hoy, "Y-m-d");
    $intervalo = date_diff($hoy, $factura);
    echo "<br>Le restan " . $intervalo->format("%R%a días para pagar");

    $navidad = date_create("2023-12-25");
    $diasParaNavidad = date_diff($hoy, $navidad);
    echo "<br>El día de hoy es: " . date_format($hoy, "Y-m-d");
    echo "<br>Faltan " . $diasParaNavidad->format("%R%a días para Navidad");
